...
def test_effort_profiles_match_the_chat_effort_map() -> None:
    ...
    assert (instant.max_agent_tool_steps, instant.max_output_tokens, instant.timeout_seconds) == (4, 4_096, 20)
    assert (medium.max_agent_tool_steps, medium.max_output_tokens, medium.timeout_seconds) == (10, 12_288, 60)
    assert (high.max_tool_actions, high.max_parallel_actions, high.run_timeout_seconds) == (36, 8, 720)
...
def test_provider_output_budget_clamps_to_live_and_static_ceilings() -> None:
    ...
    assert provider_output_budget("openrouter", "some-model", 9_000) == 9_000
    assert provider_output_budget("groq", "llama-3.3-70b", 9_000) == 9_000
    # Unknown providers fall back to the conservative default ceiling.
    assert provider_output_budget("mystery", "m", 9_000) == 8_192
    ...
    assert provider_output_budget("groq", "llama-3.3-70b", 9_000, model_max_completion_tokens=0) == 9_000
