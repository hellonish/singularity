...
export function SettingsModal() {
  ...
  return (
    <div
      ...
      style={{ position: 'fixed', inset: 0, zIndex: 60, backgroundColor: 'rgba(6,9,14,0.58)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: isMobile ? 0 : '24px' }}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        data-screen-label="Settings"
        className="animate-sg-pop"
        style={{ width: isMobile ? '100%' : 'min(760px, 100%)', height: isMobile ? '100%' : '86vh', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--surface)', border: isMobile ? 'none' : '1px solid var(--border-strong)', borderRadius: isMobile ? 0 : '20px', boxShadow: 'var(--shadow)', overflow: 'hidden' }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: isMobile ? '16px' : '20px 24px', borderBottom: '1px solid var(--border)' }}>
          <div>
            ...
            <h2 style={{ margin: 0, fontSize: 'clamp(19px,5vw,23px)', fontWeight: 400, letterSpacing: '-.01em' }}>{activeSettingsTab === 'models' ? 'Model providers' : 'Appearance'}</h2>
          </div>
          <button
            onClick={() => setSettingsOpen(false)}
            style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', width: '34px', height: '34px', border: '1px solid var(--border)', backgroundColor: 'var(--surface-2)', color: 'var(--text-dim)', borderRadius: '9px', cursor: 'pointer', flexShrink: 0 }}
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
          </button>
        </div>
        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', minHeight: 0, flex: 1 }}>
          <div style={{ width: isMobile ? '100%' : '212px', flexShrink: 0, borderRight: isMobile ? 'none' : '1px solid var(--border)', borderBottom: isMobile ? '1px solid var(--border)' : 'none', padding: '12px', overflowX: isMobile ? 'auto' : undefined, overflowY: isMobile ? undefined : 'auto', display: 'flex', flexDirection: isMobile ? 'row' : 'column', gap: '4px' }}>
            <button
              onClick={() => setActiveSettingsTab('models')}
              style={{ display: 'flex', alignItems: 'center', gap: '10px', width: isMobile ? 'auto' : '100%', flexShrink: 0, padding: '9px 12px', borderRadius: '11px', cursor: 'pointer', transition: 'all .15s', border: '1px solid transparent', color: 'var(--text)', backgroundColor: activeSettingsTab === 'models' ? 'var(--surface-2)' : 'transparent', borderColor: activeSettingsTab === 'models' ? 'var(--border)' : 'transparent' }}
            >
              ...
              <span style={{ flex: 1, minWidth: 0, textAlign: 'left', fontSize: '14px', fontWeight: 500, whiteSpace: 'nowrap' }}>Models</span>
            </button>
            <button
              onClick={() => setActiveSettingsTab('appearance')}
              style={{ display: 'flex', alignItems: 'center', gap: '10px', width: isMobile ? 'auto' : '100%', flexShrink: 0, padding: '9px 12px', borderRadius: '11px', cursor: 'pointer', transition: 'all .15s', border: '1px solid transparent', color: 'var(--text)', backgroundColor: activeSettingsTab === 'appearance' ? 'var(--surface-2)' : 'transparent', borderColor: activeSettingsTab === 'appearance' ? 'var(--border)' : 'transparent' }}
            >
              ...
              <span style={{ flex: 1, minWidth: 0, textAlign: 'left', fontSize: '14px', fontWeight: 500, whiteSpace: 'nowrap' }}>Appearance</span>
            </button>
          </div>
          <div style={{ flex: 1, minWidth: 0, padding: isMobile ? '16px' : '24px', overflowY: 'auto' }}>
